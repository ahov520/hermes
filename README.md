...
Hermes Agent 网关（`api_server`，默认端口 8642，API Key 鉴权）的 Android 手机客户端。
功能与界面对标 Hermes Studio（对话流式、工具追踪、审批、斜杠指令、用量统计、技能/模型浏览、墨水风主题），并加入移动端原生创新：系统通知、审批震动、图片发送、草稿、置顶、多连接配置。

## 功能

| 页面 | 能力 | 对应 API |
|---|---|---|
| 聊天 | SSE 流式对话、工具调用折叠卡、思考折叠、审批（经任务页）、停止、图片发送、会话分组/搜索/置顶/重命名/分叉/结束/删除、草稿、斜杠指令、连接状态灯 | `/api/sessions/*` |
| 终端 | 伪终端（run 代执行 shell，原样返回 stdout/stderr，历史 50 条） | `/v1/runs/*` |
| 任务 | run 提交/实时事件流/审批（四选项）/中断，系统通知 + 震动 | `/v1/runs/*` |
| 定时 | cron 任务管理、调度中文描述、倒计时、技能/重复表单、最近执行摘要 | `/api/jobs/*` |
| 更多→技能 | 技能库浏览（分类分组 + 搜索） | `/v1/skills` |
| 更多→模型 | 模型/路由别名、工具集、API 能力清单 | `/v1/models` 等 |
| 更多→用量 | token/费用聚合、30 天输出柱状图、模型分布（客户端聚合） | `/api/sessions` |
| 更多→设置 | 多连接配置、外观（墨水风/彩色 + 深浅色）、服务器信息、关于 | `/health/*`, `/v1/*` |
